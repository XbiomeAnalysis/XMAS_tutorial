...
```{r,, warning=FALSE, message=FALSE}
ps_genus_rb <- summarize_taxa(dada2_ps, taxa_level = "Genus", absolute = FALSE)
ps_genus_rb
...
```{r, warning=FALSE, message=FALSE}
ps_trim_mgs <- run_trim(metaphlan2_ps, cutoff = 0.4, trim = "feature")
ps_trim_mgs
```
</details>
...
```{r, warning=FALSE, message=FALSE}
data("dada2_ps")
ps_phylum <- summarize_taxa(dada2_ps, taxa_level = "Phylum")
ps_phylum
```

* amplicon sequencing: Order
```{r, warning=FALSE, message=FALSE}
ps_order <- summarize_taxa(dada2_ps, taxa_level = "Order")
ps_order
```
...
```{r, warning=FALSE, message=FALSE}
ps_family <- summarize_taxa(dada2_ps, taxa_level = "Family")
ps_family
```

* amplicon sequencing: Genus
```{r, warning=FALSE, message=FALSE}
ps_genus <- summarize_taxa(dada2_ps, taxa_level = "Genus")
ps_genus
```
...
```{r, warning=FALSE, message=FALSE}
ps_genus_mgs <- summarize_taxa(metaphlan2_ps, taxa_level = "Genus")
ps_genus_mgs

ps_species_mgs <- summarize_taxa(metaphlan2_ps, taxa_level = "Species")
ps_species_mgs
```
</details>
...
## Removing samples of specific group in phyloseq-class object
...
## Checking samples' name whether start with character

Recommending users using samples' names whose 1st label is the character not the numeric label.
```{r,, warning=FALSE, message=FALSE}
metadata <- data.frame(matrix(1:9, nrow = 3, ncol = 3))
rownames(metadata) <- seq(nrow(metadata))
head(metadata)

check_sample_names(data_sam = metadata)
```
