...
library(conflicted)
conflicted::conflict_prefer("normalize", "XMAS")
```
...
dada2_ps_transform <- transform_abundances(dada2_ps_genus, transform = "log10")
...
metaphlan2_ps_transform_mgs <- transform_abundances(metaphlan2_ps_species, transform = "log10")
...
```{r, warning=FALSE, message=FALSE}
metaphlan2_ps_trim_mgs <- run_trim(metaphlan2_ps_remove_BRS, cutoff = 0.4, trim = "feature")
...
```{r, warning=FALSE, message=FALSE}
dada2_ps_phylum <- summarize_taxa(dada2_ps_remove_BRS, taxa_level = "Phylum")
dada2_ps_phylum
```

* amplicon sequencing: Order
```{r, warning=FALSE, message=FALSE}
dada2_ps_order <- summarize_taxa(dada2_ps_remove_BRS, taxa_level = "Order")
dada2_ps_order
```
...
```{r, warning=FALSE, message=FALSE}
dada2_ps_family <- summarize_taxa(dada2_ps_remove_BRS, taxa_level = "Family")
dada2_ps_family
```
...
```{r, warning=FALSE, message=FALSE}
metaphlan2_ps_genus_mgs <- summarize_taxa(metaphlan2_ps_remove_BRS, taxa_level = "Genus")
metaphlan2_ps_genus_mgs

metaphlan2_ps_species_mgs <- summarize_taxa(metaphlan2_ps_remove_BRS, taxa_level = "Species")
metaphlan2_ps_species_mgs
```
</details>
...
```{r, warning=FALSE, message=FALSE}
# relative abundance
dada2_ps_genus_rb <- summarize_taxa(dada2_ps_remove_BRS, taxa_level = "Genus", absolute = FALSE)
dada2_ps_genus_LRA <- summarize_LowAbundance_taxa(ps_genus_rb, cutoff = 1e-04, unclass = TRUE)
dada2_ps_genus_LRA 
tail(phyloseq::taxa_names(dada2_ps_genus_LRA))

# absolute abundance
dada2_ps_genus_counts <- summarize_LowAbundance_taxa(dada2_ps_genus, cutoff = 10)
...
```{r, warning=FALSE, message=FALSE}
metaphlan2_ps_genus_LRA_mgs <- summarize_LowAbundance_taxa(metaphlan2_ps_genus_mgs, cutoff = 1e-04, unclass = TRUE)
metaphlan2_ps_genus_LRA_mgs
tail(phyloseq::taxa_names(metaphlan2_ps_genus_LRA_mgs))
```
</details>
